...
fun BrowseSourceContent(
    ...
    if (animeList.itemCount == 0 && animeList.loadState.refresh is LoadState.Loading) {
        ...
        // KMK -->
        browseSourceState.animeDisplayingList.clear()
        // KMK <--
        return
    }

    // KMK -->
    for (idx in browseSourceState.animeDisplayingList.size..<animeList.itemCount) {
        animeList[idx]?.collectAsState()?.value?.first?.let { anime ->
            if (!browseSourceState.animeDisplayingList.map { it.id }.contains(anime.id)) {
                browseSourceState.animeDisplayingList.add(anime)
            }
        }
    }
    // KMK <--

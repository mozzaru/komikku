...
class AnimeScreen(
    ...
    override fun Content() {
        ...
        val screenModel = rememberScreenModel {
            AnimeScreenModel(context, lifecycleOwner.lifecycle, animeId, fromSource, smartSearchConfig != null)
        }
        ...
        val showingRelatedAnimesScreen = rememberSaveable { mutableStateOf(false) }

        BackHandler(enabled = bulkFavoriteState.selectionMode || showingRelatedAnimesScreen.value) {
            when {
                bulkFavoriteState.selectionMode -> bulkFavoriteScreenModel.toggleSelectionMode()
                showingRelatedAnimesScreen.value -> showingRelatedAnimesScreen.value = false
            }
        }

        val content = @Composable {
            Crossfade(
                targetState = showingRelatedAnimesScreen.value,
                label = "anime_related_crossfade",
            ) { showRelatedAnimesScreen ->
                when (showRelatedAnimesScreen) {
                    true -> RelatedAnimesScreen(
                        screenModel = screenModel,
                        successState = successState,
                        bulkFavoriteScreenModel = bulkFavoriteScreenModel,
                        navigateUp = { showingRelatedAnimesScreen.value = false },
                        navigator = navigator,
                        scope = scope,
                    )
                    false -> AnimeDetailContent(
                        context = context,
                        screenModel = screenModel,
                        successState = successState,
                        bulkFavoriteScreenModel = bulkFavoriteScreenModel,
                        showRelatedAnimesScreen = { showingRelatedAnimesScreen.value = true },
                        navigator = navigator,
                        scope = scope,
                    )
                }
    ...
    fun AnimeDetailContent(
        ...
        AnimeScreen(
            ...
            onEpisodeClicked = { openEpisode(context, it) },
            ...
            onContinueReading = { continueReading(context, screenModel.getNextUnreadEpisode()) },
            ...
            // SY -->
            onMigrateClicked = { migrateAnime(navigator, screenModel.anime!!) }.takeIf { successState.anime.favorite },
            ...
            onOpenPagePreview = { page ->
                openPagePreview(context, successState.episodes.minByOrNull { it.episode.sourceOrder }?.episode, page)
            },
            onMorePreviewsClicked = { openMorePagePreviews(navigator, successState.anime) },
            // SY <--
            onMultiBookmarkClicked = screenModel::bookmarkEpisodes,
            onMultiMarkAsReadClicked = screenModel::markEpisodesRead,
            onMarkPreviousAsReadClicked = screenModel::markPreviousEpisodeRead,
            ...
            onRelatedAnimesScreenClick = {
                if (successState.isRelatedAnimesFetched == null) {
                    screenModel.screenModelScope.launchIO { screenModel.fetchRelatedAnimesFromSource(onDemand = true) }
                }
                showRelatedAnimesScreen()
            },
            onRelatedAnimeClick = { navigator.push(AnimeScreen(it.id, true)) },
            onRelatedAnimeLongClick = { bulkFavoriteScreenModel.addRemoveAnime(it, haptic) },
            onSourceClick = {
        ...
        when (val dialog = successState.dialog) {
            ...
            is AnimeScreenModel.Dialog.DuplicateAnime -> {
                DuplicateAnimeDialog(
                    ...
                    onMigrate = {
                        // SY -->
                        migrateAnime(navigator, dialog.duplicate, screenModel.anime!!.id)
                        // SY <--
                    },
                    // KMK -->
    ...
    private fun continueReading(context: Context, unreadEpisode: Episode?) {
        if (unreadEpisode != null) openEpisode(context, unreadEpisode)
    }

    private fun openEpisode(context: Context, episode: Episode) {
     ...
     */
    private fun migrateAnime(navigator: Navigator, anime: Anime, toAnimeId: Long? = null) {
        // SY -->
    ...
    private fun openMergedAnimeWebview(context: Context, navigator: Navigator, mergedAnimeData: MergedAnimeData) {
        val sourceManager: SourceManager = Injekt.get()
        val mergedAnime = mergedAnimeData.anime.values.filterNot { it.source == MERGED_SOURCE_ID }
        ...
        MaterialAlertDialogBuilder(context)
            ...
            .setSingleChoiceItems(
                Array(mergedAnime.size) { index -> sources[index].toString() },
                -1,
            ) { dialog, index ->
                dialog.dismiss()
                openAnimeInWebView(navigator, mergedAnime[index], sources[index] as? HttpSource)
            }
    ...
    private fun openPagePreview(context: Context, episode: Episode?, page: Int) {
        episode ?: return
        context.startActivity(ReaderActivity.newIntent(context, episode.animeId, episode.id, page))
    }
    // SY <--
    ...
    private fun mergeWithAnother(
        ...
        launchUI {
            try {
                val mergedAnime = withNonCancellableContext {
                    smartSearchMerge(anime, smartSearchConfig?.origAnimeId!!)
                }
                ...
                if (navigator.lastItem !is AnimeScreen) {
                    navigator push AnimeScreen(mergedAnime.id)
                } else {
                    // KMK <--
                    navigator replace AnimeScreen(mergedAnime.id)
                }

...
class BackupCreator(
    ...
    suspend fun backup(uri: Uri, options: BackupOptions): String {
        ...
        try {
            ...
            val nonFavoriteAnime = if (options.readEntries) animeRepository.getWatchedAnimeNotInLibrary() else emptyList()
            // SY -->
            val mergedAnime = getMergedAnime.await()
            // SY <--
            val backupAnime =
                backupAnimes(getFavorites.await() + nonFavoriteAnime /* SY --> */ + mergedAnime /* SY <-- */, options)
    ...
    fun backupSources(animes: List<BackupAnime>): List<BackupSource> {

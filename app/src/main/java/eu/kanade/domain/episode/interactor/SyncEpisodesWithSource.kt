...
class SyncEpisodesWithSource(
    ...
    suspend fun await(
        ...
        for (sourceEpisode in sourceEpisodes) {
            ...
            val episodeNumber = tachiyomi.domain.episode.service.EpisodeRecognition.parseEpisodeNumber(
                anime.title,
                episode.name,
        ...
        val deletedReadEpisodeNumbers = TreeSet<Double>()
        val deletedBookmarkedEpisodeNumbers = TreeSet<Double>()

        removedEpisodes.forEach { episode ->
            if (episode.seen) deletedReadEpisodeNumbers.add(episode.episodeNumber)
        ...
        var updatedToAdd = newEpisodes.map { toAddItem ->
            ...
            episode = episode.copy(
                seen = episode.episodeNumber in deletedReadEpisodeNumbers,

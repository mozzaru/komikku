...
class GetMergedEpisodesByAnimeId(
    ...
    suspend fun subscribe(
        ...
        return try {
            episodeRepository.getMergedEpisodeByAnimeIdAsFlow(animeId, applyScanlatorFilter)
                .combine(getMergedReferencesById.subscribe(animeId)) { episodes, references ->
    ...
    private fun transformMergedEpisodes(
        animeReferences: List<tachiyomi.domain.anime.model.MergedAnimeReference>,
    ...
    private fun dedupeEpisodeList(
        ...
        return when (animeReferences.firstOrNull { it.animeSourceId == MERGED_SOURCE_ID }?.episodeSortMode) {
            tachiyomi.domain.anime.model.MergedAnimeReference.CHAPTER_SORT_NO_DEDUPE, tachiyomi.domain.anime.model.MergedAnimeReference.CHAPTER_SORT_NONE -> episodeList
            tachiyomi.domain.anime.model.MergedAnimeReference.CHAPTER_SORT_PRIORITY -> dedupeByPriority(animeReferences, episodeList)
            tachiyomi.domain.anime.model.MergedAnimeReference.CHAPTER_SORT_MOST_CHAPTERS -> {
                findSourceWithMostEpisodes(episodeList)?.let { animeId ->
                    episodeList.filter { it.animeId == animeId }
                } ?: episodeList
            }
            tachiyomi.domain.anime.model.MergedAnimeReference.CHAPTER_SORT_HIGHEST_CHAPTER_NUMBER -> {

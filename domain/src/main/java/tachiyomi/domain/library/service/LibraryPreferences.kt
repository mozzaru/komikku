...
class LibraryPreferences(
    ...
    fun setChapterSettingsDefault(anime: Anime) {
        filterChapterByRead().set(anime.unreadFilterRaw)
        filterChapterByDownloaded().set(anime.downloadedFilterRaw)
        filterChapterByBookmarked().set(anime.bookmarkedFilterRaw)
        sortChapterBySourceOrNumber().set(anime.sorting)
        displayChapterByNameOrNumber().set(anime.displayMode)
        sortChapterByAscendingOrDescending().set(
            if (anime.sortDescending()) Anime.CHAPTER_SORT_DESC else Anime.CHAPTER_SORT_ASC,
        )
    }
